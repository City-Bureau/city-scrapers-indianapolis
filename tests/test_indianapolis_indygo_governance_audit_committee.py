...
from city_scrapers.spiders.indianapolis_indygo_governance_audit_committee import (
    IndianapolisIndygoGovernanceAuditCommitteeSpider,
)
...
test_response = file_response(
    join(
        dirname(__file__),
        "files",
        "indianapolis_indygo_governance_audit_committee.html",
    ),
    url="https://www.indygo.net/about-indygo/board-of-directors/",
)
spider = IndianapolisIndygoGovernanceAuditCommitteeSpider()
...
def test_id():
    assert (
        parsed_items[0]["id"]
        == "indianapolis_indygo_governance_audit_committee/202301191300/x/indygo_governance_audit_committee"  # noqa
    )
